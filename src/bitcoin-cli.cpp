...
std::string HelpMessageCli()
{
    ...
    strUsage += "  -rpcport=<port>        " + strprintf(_("Connect to JSON-RPC on <port> (default: %u or testnet: %u)"), 6262, 6262) + "\n";
...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("Neko Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  neko-cli [options] <command> [params]  " + _("Send command to Neko Core") + "\n" +
                  "  neko-cli [options] help                " + _("List commands") + "\n" +
                  "  neko-cli [options] help <command>      " + _("Get help for a command") + "\n";
...
UniValue CallRPC(const string& strMethod, const UniValue& params)
{
    ...
    // neko:
    // we get nekod output (possibly binary) of .write() that either:
