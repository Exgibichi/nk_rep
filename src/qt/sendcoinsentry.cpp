    ...
    model(0)
{
    ...
    ui->payTo->setValidator(0);  // neko: disable validator so that we can type names
    ui->payAmountExch->setValidator( new QDoubleValidator(0, 1e20, 8, this) );
    qsExchInfo = "<html><head/><body><p><span style=\" font-weight:600;\">"+
            tr("WARNING: You're using external service! Neko is not responsible for functionality and correct behavior of this service.")+"</span><br/>"+
...
void SendCoinsEntry::on_requestPaymentButton_clicked()
{
    ...
    multimap<double, pair<Exch *, bool> > mapExch;   // double: nk per 1 btc
    ...
    foreach (Exch* exch, eBox.m_v_exch)
    {
        ...
        mapExch.insert(pair<double, pair<Exch *, bool> >(exch->EstimatedNK(dPay), pair<Exch *, bool>(exch, valid)));
    }
    ...
    foreach (const PAIRTYPE(double, PAIRTYPE(Exch *, bool)) &p, mapExch)
    {
        ...
        if (valid)
            qsEntry = QString::number(p.first)+"nk ["+QString::fromStdString(p.second.first->Name())+"]";
        else
