...
//-----------------------------------------------------
// Get input path within server, like: /api/marketinfo/nk_btc.json
...
//-----------------------------------------------------
// Returns extimated NK to pay for specific pay_amount
// Must be called after MarketInfo
double Exch::EstimatedNK(double pay_amount) const {
  return (m_rate <= 0.0)?
...
UniValue Exch::httpsFetch(const char *get, const UniValue *post) {
  ...
  stream << reqType << (get? get : "/") << " HTTP/1.1\r\n"
         << "Host: " << Host() << "\r\n"
         << "User-Agent: neko-json-rpc/" << FormatFullVersion() << "\r\n";
...
string ExchCoinReform::MarketInfo(const string &currency) {
  try {
    ...
    const UniValue mi(RawMarketInfo("/api/marketinfo/nk_" + currency + ".json"));
...
//coinReform
//{"pair":"NK_BTC","rate":"0.00016236","limit":"0.01623600","min":"0.00030000","minerFee":"0.00050000"}
...
string ExchCoinReform::Send(const string &to, double amount) {
  ...
  try {
    ...
    m_depAddr  = Resp["deposit"].get_str();			// Address to pay NK
    m_outAddr  = Resp["withdrawal"].get_str();			// Address to pay from exchange
    m_depAmo   = atof(Resp["deposit_amount"].get_str().c_str());// amount in NK
    ...
    // Adjust deposit amount to 1NKent, upward
